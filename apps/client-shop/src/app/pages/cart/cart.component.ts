...
@Component({
  ...
  <div class="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8">
        ...
        <div class="mb-4 lg:mb-8">
          <h1 class="text-2xl lg:text-3xl font-bold text-slate-900 mb-1 lg:mb-2">Shopping Cart</h1>
          <p class="text-sm lg:text-base text-slate-600">Review and manage your order</p>
        </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8">
            ...
            <div class="lg:col-span-2 space-y-3 lg:space-y-4">
              <div class="bg-white rounded-lg shadow-sm">
                <div class="p-3 lg:p-6 border-b border-slate-200">
                  <h2 class="text-lg lg:text-xl font-semibold text-slate-900">Cart Items ({{ cartStore.totalItems() }})</h2>
                </div>

                <div class="divide-y divide-slate-200">
                  @for (group of cartStore.groupedItems(); track group.productId) {
                    <div class="p-3 lg:p-6 hover:bg-slate-50 transition-colors">
                      <!-- Product Header with Image and Total -->
                      <div class="flex items-start justify-between gap-3 lg:gap-4 mb-3 lg:mb-4">
                        <div class="flex items-start gap-3 lg:gap-4">
                          <!-- Product Image -->
                          <div class="flex-shrink-0 w-16 h-16 lg:w-20 lg:h-20 bg-slate-200 rounded-lg flex items-center justify-center">
                            <i class="pi pi-image text-slate-400 text-lg lg:text-xl"></i>
                          </div>

                          <!-- Product Details -->
                          <div class="flex-1 min-w-0">
                            <h3 class="text-base lg:text-lg font-semibold text-slate-900">{{ group.productName }}</h3>
                            <p class="text-slate-600 text-xs lg:text-sm mt-1">{{ group.productCode }}</p>
                            <p class="text-slate-900 font-medium text-sm lg:text-base mt-1 lg:mt-2">{{ group.unitPrice | appCurrency }} each</p>
                          </div>
                        </div>

                        <!-- Total Quantity and Price (Top Right) -->
                        <div class="text-right flex-shrink-0">
                          <div class="text-xs font-semibold text-slate-900 bg-blue-100 px-2 py-1 rounded mb-1">{{ group.totalQuantity }} items</div>
                          <div class="text-base lg:text-lg font-bold text-slate-900">{{ group.totalPrice | appCurrency }}</div>
                        </div>
                      </div>
                      ...
                      <div class="space-y-2 lg:space-y-3">
                        @for (sizeVariant of group.sizes; track sizeVariant.size) {
                          <div class="bg-slate-50 rounded-lg p-2 lg:p-3">
                            <div class="flex items-center justify-between">
                              <!-- Left: Size and Quantity Controls -->
                              <div class="flex items-center gap-3 lg:gap-4 flex-1">
                                <span class="text-xs lg:text-sm font-medium text-slate-700 min-w-16 lg:min-w-20">Size {{ sizeVariant.size }}</span>
                                ...
                                <div class="flex items-center gap-1 lg:gap-2">
                                  <p-button
                                    ...
                                    [text]="true"
                                    [rounded]="true"
                                    styleClass="!w-6 !h-6 lg:!w-7 lg:!h-7 !text-slate-600 hover:!bg-slate-200"
                                  ...
                                  <div class="flex items-center gap-1">
                                    ...
                                    <span class="text-xs lg:text-sm font-semibold text-slate-900 min-w-8 lg:min-w-10 text-center">{{ sizeVariant.quantity }}</span>
                                  </div>
                              ...
                              <!-- Right: Size Total and Remove Button -->
                              <div class="flex items-center gap-2 lg:gap-3">
                                <!-- Size Total -->
                                <span class="text-xs lg:text-sm font-bold text-slate-900">{{ sizeVariant.totalPrice | appCurrency }}</span>
                                ...
                                <p-button
                                  ...
                                  [rounded]="true"
                                  size="small"
                                  styleClass="!w-6 !h-6 lg:!w-7 lg:!h-7"
              ...
              <div class="bg-white rounded-lg shadow-sm p-3 lg:p-6">
                <p-button
                  ...
                  severity="secondary"
                  [outlined]="true"
                  styleClass="!text-sm lg:!text-base"
                  (onClick)="navigateToProducts()">
                </p-button>
              </div>
            </div>
            ...
            <div class="lg:col-span-1">
              <div class="bg-white rounded-lg shadow-sm lg:sticky lg:top-4">
                <div class="p-4 lg:p-6">
                  <h2 class="text-lg lg:text-xl font-semibold text-slate-900 mb-4 lg:mb-6">Order Summary</h2>

                    <div class="space-y-3 lg:space-y-4 mb-4 lg:mb-6">
                    <div class="flex justify-between">
                      <span class="text-sm lg:text-base text-slate-600">Subtotal ({{ cartStore.totalItems() }} items)</span>
                      <span class="text-sm lg:text-base text-slate-900 font-medium">{{ cartStore.cartSummary().subtotal | appCurrency }}</span>
                    </div>

                    <div class="flex justify-between">
                      <span class="text-sm lg:text-base text-slate-600">Shipping</span>
                      <span class="text-sm lg:text-base text-green-600 font-medium">Free</span>
                    </div>
                    ...
                    <!-- VAT Information -->
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-2 lg:p-3 mt-3 lg:mt-4">
                      <div class="flex items-start gap-2">
                        <i class="pi pi-info-circle text-blue-600 text-xs lg:text-sm mt-0.5"></i>
                        <div class="text-xs text-blue-800">
                          <p class="font-medium">Prices do NOT include VAT</p>
                          <p class="hidden lg:block">VAT will be calculated based on your location during checkout</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <p-button
                    label="Submit Order"
                    icon="pi pi-check"
                    styleClass="w-full !bg-blue-600 !border-blue-600 text-white hover:!bg-blue-700 !text-sm lg:!text-base"
                    size="large"
                  ...
                  @if (isValidatingStock()) {
                    ...
                    <p class="text-xs text-slate-500 mt-2 lg:mt-3 text-center">
